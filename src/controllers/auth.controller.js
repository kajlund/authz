...
export function getAuthController(cnf, log) {
  ...
  return {
    changePassword: asyncHandler(async (req, res) => {
      ...
      res
        .status(codes.OK)
        .json(new ApiResponse(codes.OK, {}, 'Password successfully changed'));
    }),
      ...
      res
        ...
        .json(
          new ApiResponse(
            codes.OK,
            { data: user },
            `User information for ${user.alias}`,
          ),
        );
    }),
    login: asyncHandler(async (req, res) => {
      ...
      res
        ...
        .json(
          new ApiResponse(codes.OK, data, `User ${data.user.alias} logged in`),
        );
    }),
    ...
    register: asyncHandler(async (req, res) => {
      ...
      res
        ...
        .json(
          new ApiResponse(codes.CREATED, data, `User successfully registered`),
        );
    }),
    resendVerifyEmail: asyncHandler(async (req, res) => {
      const verificationPath = `${req.protocol}://${req.get('host')}/api/v1/auth/verify`;
      const data = await svcUser.resendVerification(
        req.user.id,
        verificationPath,
      );
      res
        .status(codes.OK)
        .json(new ApiResponse(codes.OK, data, 'Verification email was sent'));
    }),
    ...
    sendPasswordResetMail: asyncHandler(async (req, res) => {
      ...
      res
        .status(codes.OK)
        .json(
          new ApiResponse(codes.OK, data, 'Password reset email has been sent'),
        );
    }),
    verifyEmail: asyncHandler(async (req, res) => {
      ...
      res
        .status(codes.OK)
        .json(new ApiResponse(codes.OK, { user }, 'User account verified'));
    }),
  };
}
